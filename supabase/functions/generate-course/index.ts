...
1. ПЕРВЫЙ БЛОК — "design" с ВВЕДЕНИЕМ: badge урока + heading + text + image (ОБЯЗАТЕЛЬНО с imageDescription для AI-генерации!)
2. СРЕДНИЕ БЛОКИ — обучающий контент и практика (design, квизы, примеры)
3. ПОСЛЕДНИЙ БЛОК — "design" с ИТОГАМИ: heading "Отлично!" + badge-и с полученными навыками + animation (animationKeyword: "success", "trophy")

## ДОСТУПНЫЕ ТИПЫ БЛОКОВ (выбирай самые подходящие!):

### КОНТЕНТНЫЕ:
- "design" — составной блок с sub-blocks (для красивых информационных слайдов). ОБЯЗАТЕЛЬНО используй animation sub-block в первом и последнем слайде урока!
- "text" — простой текст
...
1. ПЕРВЫЙ БЛОК каждого урока = design с ВВЕДЕНИЕМ (badge урока + heading + text + image с imageDescription для AI!)
2. СРЕДНИЕ БЛОКИ = обучающий контент (design, квизы разных типов)
3. ПОСЛЕДНИЙ БЛОК каждого урока = design с ИТОГАМИ (animation + heading "Отлично!" + text + badge-и с навыками урока)

## КРИТИЧЕСКОЕ ПРАВИЛО ДЛЯ КВИЗОВ:
⚠️ КВИЗЫ ДОЛЖНЫ ПРОВЕРЯТЬ ТОЛЬКО ТО, ЧТО УЖЕ БЫЛО ИЗУЧЕНО!
- Каждый квиз проверяет информацию из ПРЕДЫДУЩИХ блоков этого или прошлых уроков
- НИКОГДА не задавай вопрос о том, что ещё не было объяснено
- Сначала обучающий контент (design с фактами) → потом квиз по этому контенту
- Если квиз про термин — этот термин должен быть введён раньше
- Если квиз про процесс — этапы процесса должны быть описаны раньше
