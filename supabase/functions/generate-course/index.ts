...
- ОДНА мысль = ОДИН text блок (не перечисляй несколько пунктов в одном блоке!)
...
- Если нужно перечислить — используй НЕСКОЛЬКО text блоков или badge
...
- МИНИМУМ 4-5 саб-блоков на design слайд — ИСПОЛЬЗУЙ ВСЁ ПРОСТРАНСТВО ЭКРАНА!
...
- НЕ БОЛЕЕ 5-8 image на ВЕСЬ курс (используй animation как альтернативу!)
...
- После heading/text ОБЯЗАТЕЛЬНО добавь: image, animation, table, badge или divider
- ❌ ПЛОХО: heading → text → text → text
- ✅ ХОРОШО: heading → text → image → text → badge
- ✅ ХОРОШО: heading → animation → text → table
- ✅ ХОРОШО: badge → heading → text → divider → text → animation

## ВИЗУАЛЬНОЕ РАЗНООБРАЗИЕ:
- Каждый design блок должен иметь хотя бы 1 визуальный элемент: image, animation, table, или badge
- animation — лёгкая альтернатива image (не расходует лимит картинок!)
- table — отлично для сравнений, характеристик, списков
- badge — для ключевых понятий, терминов, навыков
