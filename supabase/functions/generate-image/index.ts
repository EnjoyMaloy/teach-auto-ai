...
- The image should visually represent the concept, not explain it with text`;
    ...
    console.log(`Generating image via Imagen 3 for: ${prompt.substring(0, 50)}...`);

    // Use Imagen 3 model for image generation
    const MODEL = "imagen-3.0-generate-002";
    
    const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${MODEL}:predict?key=${GEMINI_API_KEY}`, {
      method: "POST",
      headers: {
    ...
    if (!response.ok) {
      ...
      if (response.status === 429) {
        return new Response(
          JSON.stringify({ error: "Превышен лимит запросов Imagen API. Попробуйте позже." }),
          { status: 429, headers: { ...corsHeaders, "Content-Type": "application/json" } }
        );
      }
      if (response.status === 403 || response.status === 400) {
        return new Response(
          JSON.stringify({ error: "Ошибка Imagen API. Проверьте настройки ключа и биллинг." }),
    ...
    const imagePrediction = predictions[0];

    if (!imagePrediction?.bytesBase64Encoded) {
      console.log("No image generated by Imagen, returning null");
      return new Response(
    ...
    const base64Data = imagePrediction.bytesBase64Encoded;
    const mimeType = imagePrediction.mimeType || "image/png";
