...
- Resolution: high detail suitable for educational content`;
    ...
    console.log(`Generating image via Imagen 4.0 Fast for: ${(slideContext || prompt).substring(0, 60)}...`);

    // Use Imagen 4.0 Fast model
    const MODEL = "imagen-4.0-fast-generate-001";
    
    const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${MODEL}:generateImages?key=${GEMINI_API_KEY}`, {
      method: "POST",
      headers: {
    ...
    const imagePart = generatedImages[0];

    if (!imagePart?.image?.imageBytes) {
      console.log("No image generated by Imagen, response:", JSON.stringify(data).substring(0, 500));
      return new Response(
    ...
    const base64Data = imagePart.image.imageBytes;
