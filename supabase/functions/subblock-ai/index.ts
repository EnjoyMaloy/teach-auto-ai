...
- imageSize ОБЯЗАТЕЛЬНО указывай! Допустимые значения: 'small', 'medium', 'large', 'full'
- Если в исходном блоке картинка имеет imageSize — сохрани его при замене
...
async function generateImage(description: string, apiKey: string, imageSize?: string): Promise<string | null> {
  try {
    ...
    // Map imageSize to appropriate aspect ratio
    // small/medium -> 1:1 (compact), large/full -> 9:16 (tall)
    const aspectRatio = (imageSize === 'large' || imageSize === 'full') ? '9:16' : '1:1';
