...
async function generateImage(description: string, apiKey: string, imageSize?: string): Promise<string | null> {
  try {
    console.log("Generating image for:", description, "size:", imageSize);
    
    // All images are always square (1:1)
...
- Compact composition that works at small sizes`;
...
serve(async (req) => {
  ...
  try {
    ...
    if (result.newBlocks && Array.isArray(result.newBlocks)) {
      // Remove forbidden button blocks and enforce image size limits
      result.newBlocks = result.newBlocks.filter((b: any) => b.type !== 'button');
      for (const block of result.newBlocks) {
        if (block.type === 'image' && (block.imageSize === 'large' || block.imageSize === 'full')) {
          block.imageSize = 'medium';
        }
      }
      // Enforce max 5 sub-blocks
      if (result.newBlocks.length > 5) {
        result.newBlocks = result.newBlocks.slice(0, 5);
      }
      ...
      for (const block of result.newBlocks) {
        ...
        if (block.type === 'image' && block.imageDescription && !block.imageUrl) {
          ...
          promises.push(
            generateImage(block.imageDescription, GEMINI_API_KEY, block.imageSize).then(imageUrl => {
              if (imageUrl) {
