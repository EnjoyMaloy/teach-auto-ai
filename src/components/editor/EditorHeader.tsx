    ...
    <>
      <header className="h-14 border-b border-border/5 dark:border-transparent bg-secondary/50 dark:bg-black/10 backdrop-blur-sm flex items-center justify-between px-4 gap-4 relative">
        ...
        <div className="flex items-center gap-2 min-w-0">
          {/* Academy logo - visible when sidebar is closed */}
          {onToggleAISidebar && (
            <div className={cn(
              "transition-all duration-300 ease-out overflow-hidden",
              isAISidebarOpen ? "w-0 opacity-0 scale-75" : "w-8 opacity-100 scale-100"
            )}>
              <button
                onClick={onToggleAISidebar}
                className="w-8 h-8 rounded-lg bg-gradient-to-br from-[hsl(265,60%,75%)] to-[hsl(265,60%,65%)] flex items-center justify-center flex-shrink-0"
              >
